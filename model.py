...
class DIEN(tf.keras.Model):
    ...
    def concat_features(self, feature_data_dict):
        concat_list = []
    ...
    def get_emb(self, user_profile_dict, user_profile_list, click_behavior_dict, target_item_dict, noclick_behavior_dict, user_behavior_list):
        ...
        return self.concat_features(user_profile_feature_embedding), self.concat_features(target_item_feature_embedding), self.concat_features(click_behavior_embedding), self.concat_features(noclick_behavior_embedding)
    ...
    def call(self, user_profile_dict, user_profile_list, click_behavior_dict, target_item_dict, noclick_behavior_dict, user_behavior_list):
        ...
        """
        #Embedding Layer
        user_profile_embedding, target_item_embedding, click_behavior_emebedding, noclick_behavior_emebedding = self.get_emb(user_profile_dict, user_profile_list, click_behavior_dict, target_item_dict, noclick_behavior_dict, user_behavior_list)
        #GRU Layer
        ...
        #Attention Layer
        # click_mask = tf.sequence_mask(length, max(length), dtype=tf.bool)
        # click_mask = tf.tile(tf.expand_dims(click_mask, -1), (1,1,self.get_GRU_input_dim(embedding_dim_dict, user_behavior_features)))
        hist_attn = tf.nn.softmax(tf.matmul(tf.expand_dims(target_item_embedding, 1), click_gru_emb, transpose_b=True))
        #AUGRU Layer
...
if __name__ == "__main__":
    model = DIEN(dict(), dict(), list(), list())
